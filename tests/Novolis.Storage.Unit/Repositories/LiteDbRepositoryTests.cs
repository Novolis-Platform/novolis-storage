...
public sealed class LiteDbRepositoryTests : LiteDbDataStorageTestBase<ExampleClass>
{
    [Test]
    public async Task Upsert_get_delete_roundtrip()
    {
        ...
        try
        {
            ...
            var entity = new ExampleClass { Id = Guid.NewGuid(), SomeData = "LiteDb", Boolean = true };

            await repository.UpsertAsync(entity);
            var loaded = await repository.TryGetAsync(entity.Id);
            await Assert.That(loaded).IsNotNull();
            await Assert.That(loaded!.SomeData).IsEqualTo("LiteDb");
